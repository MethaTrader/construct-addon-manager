...
{
	return {
		"name":			"Phonegap IAP",				// as appears in 'insert object' dialog, can be changed as long as "id" stays the same
		"id":			"PhonegapIAP",				// this is used to identify this plugin and is saved to the project; never change it
		"version":		"2.0.25",					// (float in x.y format) Plugin version - C2 shows compatibility warnings based on this
...
AddStringParam("Product ID", "Product ID that has been purchased successfully.");
AddCondition(9, cf_trigger, "On consume success", "IAP", "On product <i>{0}</i> consume success", "Triggered when a specific product ID has been consumeed successfully.", "OnConsumeSuccess");
AddStringParam("Product ID", "Product ID that has failed to consume.");
AddCondition(10, cf_trigger, "On consume failed", "IAP", "On product <i>{0}</i> consume failed", "Triggered when a specific product ID has failed to be consumeed.", "OnConsumeFail");

//cranberrygame start
AddStringParam("Tag", "A tag, which can be anything you like, to distinguish between different restore purchases.", "\"\"");
AddCondition(11, cf_trigger, "On restore purchases success", "IAP", "On restore purchases (tag <b>{0}</b>) success", "Triggered when store listing received successfully.", "OnRestorePurchasesSuccess");
AddStringParam("Tag", "A tag, which can be anything you like, to distinguish between different restore purchases.", "\"\"");
AddCondition(12, cf_trigger, "On restore purchases failed", "IAP", "On restore purchases (tag <b>{0}</b>) failed", "Triggered when store listing failed to receive.", "OnRestorePurchasesFail");
//cranberrygame end
...
AddAction(3, af_none, "Restore purchases", "IAP", "Restore purchases (tag <i>{0}</i>)", "Restore the purchases state from the store.", "RestorePurchases");
...
AddAction(5, af_none, "Consume product", "IAP", "Consume product <b>{0}</b>", "consume a product.", "ConsumeProduct");
//cranberrygame end
